  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.container}>
      <View style={styles.card}>
        ...
        <Text style={styles.subtitle}>Log in to access your routes</Text>
        ...
        {showPassword && (
          <>
            ...
            <TouchableOpacity style={[styles.button, submitting && styles.buttonDisabled]} onPress={handleSubmit} disabled={submitting}>
              {submitting ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>{submitting ? 'Logging in…' : 'Log in'}</Text>}
            </TouchableOpacity>
            <TouchableOpacity style={styles.linkWrap} onPress={() => navigation.replace('SignUp')}>
              <Text style={styles.link}>Don't have an account? Sign up</Text>
            </TouchableOpacity>
          </>
        )}
        {!showPassword && <Text style={styles.error}>No sign-in method configured. Use email/password on the server.</Text>}
      </View>
